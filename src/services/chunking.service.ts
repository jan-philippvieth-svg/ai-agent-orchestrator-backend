...
export class ChunkingService {
  ...
  private splitOversizedChunk(chunk: string): string[] {
    if (estimateTokens(chunk) <= 800) return [chunk];
  ...
  private tailOverlap(text: string, maxTokens: number): string {
    const words = text.split(/\s+/);
    const approxWords = Math.max(1, Math.floor(maxTokens * 0.75));
    return words.slice(-approxWords).join(' ');
  }

  private isQualityChunk(chunk: string): boolean {
    const tokens = estimateTokens(chunk);
    if (tokens < 45) return false;
    const words = chunk.split(/\s+/).filter(Boolean);
    const alphaWords = words.filter((word) => /[a-zäöüß]{3,}/i.test(word));
    if (alphaWords.length < 25) return false;
    if (/^(#+\s*)?[\w\s-]{1,80}$/i.test(chunk.trim())) return false;
    return true;
  }
}
