...
interface ChatMetric {
  ...
  totalActualLlmWorkUnits: number;
  totalBaselineLlmWorkUnits: number;
  totalSavedLlmWorkUnits: number;
  totalAttemptedModels: number;
...
interface ToolMetric {
  ...
  totalRawTokensEstimated: number;
  totalInjectedTokens: number;
  totalSavedTokens: number;
}

export class MetricsService {
  private static instance: MetricsService | undefined;
  private readonly http = new Map<string, HttpMetric>();
  private readonly chat = new Map<string, ChatMetric>();
  private readonly guard = new Map<string, GuardMetric>();
  private readonly cache: CacheMetric = { hits: 0, misses: 0, writes: 0 };
  private readonly tools = new Map<string, ToolMetric>();
  private readonly guardEvents: GuardEvent[] = [];
  ...
  recordHttpRequest(method: string, route: string, statusCode: number, latencyMs: number): void {
  ...
  recordChat(event: {
    ...
    if (event.fallbackUsed) metric.fallbackCount += 1;
  ...
  recordGuard(event: GuardEvent): void {
    ...
    metric.guarded += 1;
    if (event.blocked) metric.rejected += 1;
  ...
  recordCache(event: 'hit' | 'miss' | 'write'): void {
    if (event === 'hit') this.cache.hits += 1;
    if (event === 'miss') this.cache.misses += 1;
    if (event === 'write') this.cache.writes += 1;
  }
  ...
  recordTool(event: {
    ...
    metric.count += 1;
    if (event.status === 'error') metric.errors += 1;
  ...
  efficiencySummary() {
    ...
    return {
      ...
      estimatedLlmWorkSavedPercent:
        baselineLlmWorkUnits > 0 ? Math.round((savedLlmWorkUnits / baselineLlmWorkUnits) * 1000) / 10 : 0,
      tokensEstimated,
      baselineTokens,
      savedTokens,
  ...
  renderPrometheus(): string {
    ...
    for (const [key, metric] of this.http.entries()) {
      ...
      lines.push(
        `ai_agent_orchestrator_http_requests_total{method="${method}",route="${route}",status="${statusCode}"} ${metric.count}`,
      );
    }
    ...
    for (const [key, metric] of this.http.entries()) {
      const [method, route, statusCode] = key.split('|');
      lines.push(
        `ai_agent_orchestrator_http_request_latency_ms_total{method="${method}",route="${route}",status="${statusCode}"} ${Math.round(metric.totalLatencyMs)}`,
      );
    }
    ...
    for (const [key, metric] of this.chat.entries()) {
      ...
      lines.push(
        `ai_agent_orchestrator_chat_requests_total{classification="${classification}",selected_model="${selectedModel}",retrieval_used="${retrievalUsed}"} ${metric.count}`,
      );
    }
    ...
    for (const [key, metric] of this.chat.entries()) {
      const [classification, selectedModel, retrievalUsed] = key.split('|');
      lines.push(
        `ai_agent_orchestrator_chat_tokens_estimated_total{classification="${classification}",selected_model="${selectedModel}",retrieval_used="${retrievalUsed}"} ${metric.totalTokensEstimated}`,
      );
    }
    ...
    for (const [key, metric] of this.chat.entries()) {
      const [classification, selectedModel, retrievalUsed] = key.split('|');
      lines.push(
        `ai_agent_orchestrator_chat_baseline_tokens_total{classification="${classification}",selected_model="${selectedModel}",retrieval_used="${retrievalUsed}"} ${metric.totalBaselineTokens}`,
      );
    }
    ...
    for (const [key, metric] of this.chat.entries()) {
      const [classification, selectedModel, retrievalUsed] = key.split('|');
      lines.push(
        `ai_agent_orchestrator_chat_saved_tokens_total{classification="${classification}",selected_model="${selectedModel}",retrieval_used="${retrievalUsed}"} ${metric.totalSavedTokens}`,
      );
    }
    ...
    for (const [key, metric] of this.chat.entries()) {
      const [classification, selectedModel, retrievalUsed] = key.split('|');
      lines.push(
        `ai_agent_orchestrator_chat_chunks_used_total{classification="${classification}",selected_model="${selectedModel}",retrieval_used="${retrievalUsed}"} ${metric.totalChunksUsed}`,
      );
    }
    ...
    for (const [key, metric] of this.guard.entries()) {
      ...
      lines.push(
        `ai_agent_orchestrator_guard_rejections_total{reason="${reason}",category="${category}"} ${metric.rejected}`,
      );
    }
    ...
    for (const [key, metric] of this.tools.entries()) {
      const [name, status] = key.split('|');
      lines.push(
        `ai_agent_orchestrator_tool_latency_ms_total{name="${name}",status="${status}"} ${Math.round(metric.totalLatencyMs)}`,
      );
    }
    ...
    for (const [key, metric] of this.tools.entries()) {
      const [name, status] = key.split('|');
      lines.push(
        `ai_agent_orchestrator_tool_raw_tokens_estimated_total{name="${name}",status="${status}"} ${metric.totalRawTokensEstimated}`,
      );
    }
    ...
    for (const [key, metric] of this.tools.entries()) {
      const [name, status] = key.split('|');
      lines.push(
        `ai_agent_orchestrator_tool_injected_tokens_total{name="${name}",status="${status}"} ${metric.totalInjectedTokens}`,
      );
    }
    ...
    for (const [key, metric] of this.tools.entries()) {
      const [name, status] = key.split('|');
      lines.push(
        `ai_agent_orchestrator_tool_saved_tokens_total{name="${name}",status="${status}"} ${metric.totalSavedTokens}`,
      );
    }
