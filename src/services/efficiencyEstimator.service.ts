...
import type { ModelSize } from '../types/index.js';
...
export interface EfficiencyEstimate {
  ...
  assumptions: {
    ...
    modelWorkFactors: Record<ModelSize, number>;
    embeddingWorkUnits: number;
    classificationWorkUnits: number;
    retrievalWorkUnitsPerChunk: number;
  };
}

export class EfficiencyEstimatorService {
  estimate(input: EfficiencyInput): EfficiencyEstimate {
    ...
    const orchestrationOverhead =
      config.efficiency.classificationWorkUnits +
  ...
  private modelWorkFactors(): Record<ModelSize, number> {
    return {
