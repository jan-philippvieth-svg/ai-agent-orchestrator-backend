        ...
        <tbody>
          ${latest.results
            .map((item) => {
              ...
              return `<tr><td>${item.title}</td><td>${item.selectedModel}</td><td>${item.classification}</td><td>${item.baselineTokens}</td><td>${item.actualTokens}</td><td>${item.savedTokens}</td><td>${item.latencyMs}ms</td><td>${toolNames}</td><td>${item.toolSavedTokens ?? 0}</td><td>${(item.quality as { score?: number })?.score ?? ''}</td></tr>`;
            })
            .join('')}
        </tbody>
      </table>
