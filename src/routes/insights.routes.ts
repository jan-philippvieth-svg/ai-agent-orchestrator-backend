      ...
      <table>
        ...
        <tbody>
          ${users
            .map(
              (user) =>
                `<tr><td>${user.userId}</td><td>${user.tenantId}</td><td>${user.requests}</td><td class="score">${user.avgInputQualityScore}</td><td>${user.avgAnswerValueScore}</td><td class="score">${user.avgOverallValueScore}</td><td>${user.totalSavedTokens}</td><td>${Math.round(user.retrievalUseRate * 100)}%</td></tr>`,
            )
            .join('')}
        </tbody>
      </table>
      ...
      <table>
        ...
        <tbody>
          ${state.topInteractions
            ...
            .map(
              (item) =>
                `<tr><td class="score">${item.overallValueScore}</td><td>${item.userId}</td><td>${item.messagePreview ?? item.messageHash}</td><td>${item.answerPreview ?? item.answerHash}</td><td class="muted">${item.inputSignals.join(', ')}</td></tr>`,
            )
            .join('')}
        </tbody>
      </table>
