...
export class EmbeddingService {
    ...
    async embed(text) {
        ...
        const response = await this.resilience.run('embedding:default', (signal) => fetch(config.embedding.url, {
            method: 'POST',
    ...
    async health() {
        if (config.stubExternalServices)
            return true;
        try {
            const response = await this.resilience.run('embedding:health', (signal) => fetch(config.embedding.url, {
                method: 'POST',
                headers: { 'content-type': 'application/json' },
