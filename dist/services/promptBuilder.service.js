...
export class PromptBuilderService {
    build(userMessage, chunks, toolResults = [], selectedTools = []) {
        ...
        const tools = this.reduceToolResults(toolResults)
            .map((result) => `<tool_result name="${result.name}" status="${result.status}" items_used="${result.itemsUsed}">\n${result.content}\n</tool_result>`)
            .join('\n\n');
    ...
    reduceChunks(chunks) {
        ...
        for (const chunk of chunks) {
            ...
            if (total + tokenCount > maxContextTokens)
                break;
            selected.push(chunk);
            total += tokenCount;
        }
    ...
    reduceToolResults(results) {
        ...
        for (const result of results) {
            ...
            if (total + tokenCount > maxToolTokens)
                break;
