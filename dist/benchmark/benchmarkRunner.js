...
export class BenchmarkRunner {
    ...
    toolRouter;
    toolRegistry;
    constructor(classifier = new ClassifierService(), modelRouter = new ModelRouterService(), promptGuard = new PromptGuardService(), retrieval = new RetrievalService(), prompts = new PromptBuilderService(), llm = new LlmService(), toolRouter = new ToolRouterService(), toolRegistry = new ToolRegistryService()) {
    ...
    async run() {
        ...
        for (const testCase of cases) {
            for (const scenario of ['baseline', 'rag', 'optimized']) {
    ...
    async runScenario(testCase, scenario) {
        ...
        try {
            ...
            if (guard && !guard.allowed)
                throw new Error(`Prompt rejected by guard: ${guard.reasonCode ?? 'blocked'}`);
            ...
            if (scenario === 'optimized') {
                ...
                toolResults = await this.toolRegistry.executeForChat(body, routed.selected.map((tool) => tool.name));
    ...
    buildReport(results) {
        const scenarioSummary = ['baseline', 'rag', 'optimized'].map((scenario) => {
        ...
        const modelDistribution = results.reduce((acc, item) => {
    ...
    evaluate(input) {
        ...
        const keywordHits = input.expectedKeywords.filter((keyword) => input.answer.toLowerCase().includes(keyword.toLowerCase())).length;
        ...
        const hallucinationRisk = hasContext || input.scenario === 'optimized' ? 'low' : input.scenario === 'rag' ? 'medium' : 'high';
        const contextFidelity = hasContext
            ? 'high'
            : input.scenario === 'baseline'
                ? 'low'
                : 'medium';
        const answerCompleteness = answerTokens > 80 && keywordScore >= 0.6 ? 'high' : answerTokens > 30 ? 'medium' : 'low';
        const quality = (hallucinationRisk === 'low' ? 30 : hallucinationRisk === 'medium' ? 18 : 8) +
        ...
        return {
            ...
            operatingRisk: input.scenario === 'optimized' ? 'low' : input.scenario === 'rag' ? 'medium' : 'high',
            recommendation: input.scenario === 'optimized'
    ...
    scenarioRecommendation(scenario, qualityScore, failureRate) {
        if (failureRate > 0)
            return 'weitere Optimierung nötig';
        if (scenario === 'optimized' && qualityScore >= 75)
            return 'empfohlene Enterprise-Variante';
        if (scenario === 'rag' && qualityScore >= 65)
            return 'geeignet für interne Wissensabfragen';
        if (scenario === 'baseline')
            return 'nur für einfache MVP-Tests geeignet';
        return 'weitere Optimierung nötig';
    }
    average(values) {
        if (values.length === 0)
    ...
    async withTimeout(operation, caseId, scenario) {
        let timeout;
        try {
